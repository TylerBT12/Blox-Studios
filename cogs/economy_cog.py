...
class EconomyCog(commands.Cog):
    ...
    async def balance(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(f"Balance: {bal} {cur}")
    ...
    async def daily(self, interaction: discord.Interaction):
        ...
        if cd["daily_at"]:
            ...
            if now - last < timedelta(hours=cfg["daily_cooldown_h"]):
                ...
                await interaction.response.send_message(f"Daily on cooldown: {left}", ephemeral=True)
                return
        ...
        await interaction.response.send_message(f"You claimed daily: +{cfg['daily_amount']}")
    ...
    async def work(self, interaction: discord.Interaction):
        ...
        if cd["work_at"]:
            ...
            if now - last < timedelta(minutes=cfg["work_cooldown_m"]):
                ...
                await interaction.response.send_message(f"Work on cooldown: {left}", ephemeral=True)
                return
        ...
        await interaction.response.send_message(f"Work payout: +{gain}")
    ...
    async def pay(self, interaction: discord.Interaction, user: discord.Member, amount: int):
        ...
        if amount > cfg["transfer_max"]:
            await interaction.response.send_message("Over transfer max limit.", ephemeral=True)
            return
        bal = self.bot.db.get_balance(interaction.guild_id, interaction.user.id)
        if bal < amount:
        ...
        await interaction.response.send_message(f"Paid {user.mention} {recv} (tax {tax})")
    ...
    async def buy(self, interaction: discord.Interaction, item_id: int):
        ...
        await interaction.response.send_message(f"Purchased {row['name']}")
    ...
    async def admin(self, interaction: discord.Interaction):
        await interaction.response.send_message("Economy admin panel", view=EconomyAdminView(self, interaction.guild_id), ephemeral=True)
