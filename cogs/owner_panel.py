...
class OwnerPanel(commands.Cog):
    ...
    async def givepremium(self, ctx: commands.Context, guild_id: int, duration: str, tier: str = "Gold"):
        expiry = parse_duration(duration)
        self.bot.premium.set_premium(guild_id, tier, expiry.isoformat() if expiry else None, ctx.author.id)
        await ctx.send(f"Premium granted to `{guild_id}` | tier={tier} | expires={format_dt(expiry.isoformat() if expiry else None)}")
    ...
    async def extendpremium(self, ctx: commands.Context, guild_id: int, duration: str):
        ...
        self.bot.premium.set_premium(guild_id, prem.get("tier") or "Gold", new_expiry, ctx.author.id)
    ...
    async def forceexpire(self, ctx: commands.Context, guild_id: int):
        ...
        if not prem.get("active"):
            ...
            return
        self.bot.premium.set_premium(guild_id, prem.get("tier") or "Gold", datetime.now(timezone.utc).isoformat(), ctx.author.id)
    ...
    async def premiumlist(self, ctx: commands.Context):
        ...
        if not guilds:
            ...
            return
        lines = [f"{gid}: tier={v.get('tier')} active={v.get('active')} expires={format_dt(v.get('expires_at'))}" for gid, v in guilds.items()]
        await ctx.send("\n".join(lines[:30]))
