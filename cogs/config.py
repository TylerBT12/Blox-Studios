...
class ConfigView(discord.ui.View):
    ...
    @discord.ui.select(
        ...
        options=[
            ...
            discord.SelectOption(label="Discohook", value="discohook"),
        ],
    )
    async def category_select(self, interaction: discord.Interaction, select: discord.ui.Select):
        ...
        if cat == "dashboard":
            ...
            url = data.get("discohook_webhook_url")
            subset = {"discohook_webhook_url": (url[:40] + "..." + url[-8:] if isinstance(url, str) and len(url) > 60 else url)}
        else:
            subset = data.get(cat, {})
...
class ConfigCog(commands.Cog):
    ...
    def make_config_embed(self, guild_id: int):
        ...
        url = data.get("discohook_webhook_url")
        masked = (url[:40] + "..." + url[-8:]) if isinstance(url, str) and len(url) > 60 else (url or "Not set")
        e.add_field(name="Discohook Webhook", value=f"```py\n{masked}\n```", inline=False)
        e.add_field(name="How to edit", value="Use dropdown for categories, **Edit Key** for any setting, and `/embed setwebhook` for Discohook URL.", inline=False)
        return e
