...
class APICog(commands.Cog):
    ...
    async def _run_action(self, interaction: discord.Interaction, provider: str, action_key: str, params: dict):
        ...
        if not cfg["api_actions_enabled"]:
            await interaction.response.send_message(embed=themed_embed("API Actions", "API actions disabled in config.", success=False), ephemeral=True)
            return
        if action_key not in cfg.get("allowed_actions", list(ACTIONS.keys())):
            await interaction.response.send_message(embed=themed_embed("API Actions", "Action is not allowlisted in /config.", success=False), ephemeral=True)
            return
        ...
        await interaction.response.send_message(embed=themed_embed("Game Action Result", f"{provider.upper()} action `{action_key}` result:\n{result.get('message', result)}"))

    @erlc.command(name="status")
    async def erlc_status(self, interaction: discord.Interaction):
        out = await self.bot.erlc.status()
        await interaction.response.send_message(embed=themed_embed("Provider Status", str(out)))

    @erlc.command(name="players")
    async def erlc_players(self, interaction: discord.Interaction):
        out = await self.bot.erlc.players()
        await interaction.response.send_message(embed=themed_embed("ERLC Players", f"Players: {len(out)}"))
    ...
    async def maple_players(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(embed=themed_embed("Maple Players", f"Players: {len(out)}"))

    @game.command(name="status")
    async def game_status(self, interaction: discord.Interaction):
        cfg = self.bot.db.get_guild_config(interaction.guild_id)
        mode = cfg["provider_mode"]
        await interaction.response.send_message(embed=themed_embed("Game Provider", f"Configured provider mode: {mode}"))
    ...
    async def game_players(self, interaction: discord.Interaction):
        ...
        if mode == "erlc":
            out = await self.bot.erlc.players()
            await interaction.response.send_message(embed=themed_embed("Game Players", f"Players (ERLC): {len(out)}"))
        elif mode == "maple":
            out = await self.bot.maple.players()
            await interaction.response.send_message(embed=themed_embed("Game Players", f"Players (Maple): {len(out)}"))
        else:
            e = await self.bot.erlc.players()
            m = await self.bot.maple.players()
            await interaction.response.send_message(embed=themed_embed("Game Players", f"Players ERLC={len(e)} Maple={len(m)}"))

    @game.command(name="actions")
    async def actions(self, interaction: discord.Interaction):
        cfg = self.bot.db.get_guild_config(interaction.guild_id)
        acts = allowed_actions_for_guild(cfg)
        await interaction.response.send_message(embed=themed_embed("Allowed Actions", "\n".join([f"- {a.key}: {a.display}" for a in acts])))

    @game.command(name="run")
    async def run(self, interaction: discord.Interaction, action: str, provider: str = "erlc"):
        if action not in ACTIONS:
            await interaction.response.send_message(embed=themed_embed("Game Action", "Unknown action.", success=False), ephemeral=True)
            return
