...
- Premium per-guild with expiry / unlimited (single premium plan) and license redeem
...
- License generation command path updated to `/key create <duration>` (example: `/key create 1y`)
- Advanced embed theming editable via `/config`
- Single `/config` command with one big embed + clickable category navigation
- Massive command expansion across tickets, security, roster, audit, and reports
